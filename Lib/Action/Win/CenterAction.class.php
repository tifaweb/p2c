...
//我是投资者
	public function invest(){
		...
		switch($this->_get('mid')){
			...
			$count      = M('borrow_log')->where(array('type'=>1,'uid'=>$this->_session('user_uid')))->count();
			$Page       = new Pages($count,10);
			$Page->setConfig('theme',"%first% <li class='now'>%nowPage%/%totalPage%</li> %linkPage%");
			$show       = $Page->show();
			$isclosed=$this->bidRecords(1,0,$this->_session('user_uid'),$Page->firstRow.','.$Page->listRows);
			...
			$Page->setConfig('theme',"%first% <li class='now'>%nowPage%/%totalPage%</li> %linkPage%");
			$show       = $Page->show();
			$isbid=$this->bidRecords(1,0,$this->_session('user_uid'),$Page->firstRow.','.$Page->listRows);
